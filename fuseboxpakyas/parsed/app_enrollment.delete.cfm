...
<cfset myFusebox.thisFuseaction = "delete_enrollment">
<cftry>
<cfoutput><cfinclude template="../model/enrollment/act_delete_enrollment.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 25 and right(cfcatch.MissingFileName,25) is "act_delete_enrollment.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse act_delete_enrollment.cfm in circuit m_enrollment which does not exist (from fuseaction m_enrollment.delete_enrollment).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
